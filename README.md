...
- **비관적 락(Pessimistic Lock)**을 통해 데이터 정합성을 보장하며, 대용량 트래픽 상황에서도 재고 오차 없이 1초(1s) 만에 처리를 완료했습니다.
![img_3.png](img_3.png)
...
### 💭 회고
시스템의 신뢰도는 예외 상황에서의 데이터 정합성에서 결정된다고 믿습니다. 이번 프로젝트를 통해 단순히 기능을 구현하는 것에 그치지 않고, **"왜 오차가 발생하는가?"**를 끝까지 추적하여 기술의 본질적인 동작 원리를 파악하는 과정에 집중했습니다.

비관적 락의 메커니즘을 실제 동시성 테스트로 증명해내며, 인프라와 애플리케이션 계층 간의 상호작용을 깊이 있게 이해할 수 있었습니다. 어떠한 트래픽 상황에서도 데이터가 오염되지 않는 견고한 서버를 설계하는 개발자가 되겠습니다.
## 📊 향후 개선 계획
