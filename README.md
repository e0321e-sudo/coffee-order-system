...
### 📝 데이터 설계 핵심 요약
- 매장 중심 구조: Store를 중심으로 메뉴, 키오스크, 임시 휴무 정보가 유기적으로 연결되어 매장 운영에 필요한 데이터를 관리합니다.

- 재고 및 이력 관리: 실시간 재고는 menu_stock에서 관리하며, 모든 수량 변동은 stock_history에 기록하여 데이터 추적성을 확보했습니다.

- 비관적 락(Pessimistic Lock): 동시 주문 시 발생할 수 있는 데이터 정합성 문제를 방지하기 위해 주문 및 재고 관련 로직에 DB 레벨의 비관적 락을 적용했습니다.

### 💡 데이터 저장소 분리 전략 (RDB vs NoSQL)

본 프로젝트는 데이터의 성격에 따라 저장소를 분리하여 설계했습니다.

- MySQL (RDB): 주문 정보, 포인트 잔액, 메뉴 데이터 등 정합성과 영속성이 필수적인 핵심 데이터를 관리합니다.

- Redis (NoSQL): 장바구니(Cart)와 같이 빈번한 수정이 발생하고 일정 시간(TTL 30분) 후 만료되는 휘발성 데이터를 관리합니다.

- 장바구니 데이터는 Redis Hash(cart:{kioskUuid}) 구조로 관리하여 DB의 불필요한 쓰기 부하를 줄였습니다.

이러한 이유로 위 ERD에는 Redis 데이터 모델이 포함되지 않았습니다.
...
### ✅ 전체 테스트 통과 (176 Tests Passed)
- 비즈니스 로직, 경계값 검증, 예외 상황 등 총 176개의 테스트 케이스를 구축하여 시스템 안정성을 확보했습니다.

![img_2.png](img_2.png)

### 🚀 동시성 테스트 (Concurrency Test)
- **ExecutorService**와 **CountDownLatch**를 활용하여 100건의 주문이 동시에 발생하는 상황을 검증했습니다.
  ![img_3.png](img_3.png)
* **부하 테스트 (600 VUs)**: k6를 활용한 고부하 시뮬레이션 결과, 초기 성공률 43% 확인.
  -> **최적화 후**: Kafka 통신 분리 등을 통해 성공률을 **48%**로 개선하고 병목 지점을 데이터로 특정함.
![img_4.png](img_4.png)
...
### 🔍 Troubleshooting (요약)
#### 1. Mock 환경에서의 비관적 락 동작 불가 이슈
- **문제**: 100건의 동시 주문 테스트 시 단위 테스트(Mock) 환경에서 재고 정합성이 깨지는 현상(Race Condition) 발생.
- **원인**: 비관적 락은 DB 레벨 기술이나, Mock 환경에서는 실제 잠금이 동작하지 않아 가짜 객체에 대한 동시 접근 제어 불가.
- **해결**: 테스트 데이터(재고, 포인트)를 보정하고 기술의 인프라 종속성을 이해하여, 통합 테스트 환경에서 1s 만에 100건 요청을 오차 없이 처리함을 최종 검증함.
- **[👉 상세 트러블슈팅 과정 (Velog)](https://velog.io/@sus2/Troubleshooting-100건의-동시-주문-재고-정합성을-어떻게-보장했는가-with-Mockito)**
#### 2. 고부하 상황에서의 응답 지연 및 타임아웃 이슈
- **문제**: k6 부하 테스트 시 대량의 요청이 비관적 락 대기 시간에 걸려 타임아웃 및 실패 발생.
- **원인**: 트랜잭션 내부에 포함된 Kafka 메시지 발행 등 외부 I/O 작업이 DB 커넥션 점유 시간을 불필요하게 늘림.
- **해결**: `TransactionSynchronizationManager`를 도입하여 Kafka 발행 시점을 커밋 이후로 분리. 락 점유 시간을 최소화하여 전체적인 처리량(Throughput) 개선.
---

### 💭 회고

데이터의 정합성을 최우선으로 하여 비관적 락을 도입했습니다. 부하 테스트(k6) 결과, 600 VUs 상황에서 성공률 약 48%라는 구조적 한계를 확인했습니다. 하지만 이 과정에서 네트워크 I/O 분리를 통해 성공률을 소폭 개선(5%p)하는 성과를 거두었으며, 병목 현상의 근본 원인이 DB 커넥션 점유 시간에 있음을 데이터로 증명했습니다. 완벽한 성공보다 중요한 '문제의 정확한 진단'을 경험하며, 더 높은 가용성을 위한 다음 단계(분산 락, 비동기 처리)의 필요성을 깊이 체감했습니다.

---

### 📊 향후 개선 계획
구조적 한계를 극복하기 위한 다음 단계입니다.

| 항목 | 내용 |
| --- | --- |
| **분산 락 도입** | 고가용성 환경을 위한 Redisson 기반 분산 락 전환 |
| **비동기 재고 처리** | Kafka Consumer를 활용한 재고 차감 비동기화로 응답 속도 최적화 |
| **장애 대응** | Dead Letter Queue(DLQ) 구축을 통한 메시지 처리 안정성 확보 |
| **트래픽 제어** | Redis 기반 API Rate Limiting 도입으로 시스템 과부하 방지 |
| **프론트엔드** | React/Next.js 기반 키오스크 UI 구현 및 연동 |**
